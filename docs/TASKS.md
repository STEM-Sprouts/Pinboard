...
- [x] **supabase.md** — Supabase project, `profiles`/`projects` tables, RLS policies. *Exit:* RLS tests pass (user A cannot read user B's project). → migration applied to the real project 2026-07-02; live RLS tests green (`src/supabase/rls.live.test.ts`, runs when env keys exist, skips otherwise): user B cannot see/update/delete user A's project, forged owner_id inserts rejected
- [~] **supabase.md** — Google OAuth + magic-link fallback + `/auth/callback` route. *Exit:* sign-in works; **OAuth failure does not block local use.** → auth wrappers, config-gated sign-in UI, `/auth/callback` session handling; anonymous auth verified live 2026-07-02; failure path E2E-asserted. Pending: enable the Google provider in the Supabase dashboard (user step) for real Google sign-in
- [x] **persistence.md + supabase.md** — Cloud save, local→cloud promotion, cloud→local fallback, conflict UI, project hash excluding volatile fields. *Exit:* signed-in save/load works; failed cloud write never overwrites local; hash dedup skips no-op writes. → complete 2026-07-02: "Save to my account" promotion ask (nothing uploads silently), cloud autosave after each successful local save with hash dedup (live round-trip test: save → skip → load green), conflict prompt (keep local / use cloud / save as duplicate) pauses cloud writes until resolved, failures set "your local save is safe" and never touch local state
- [x] **supabase.md** — Project list + basic account settings. *Exit:* projects appear after reload. → `/projects` merges local projects (☁ synced badge) with cloud-only rows; opening a cloud project materializes it locally first — local save stays authoritative (2026-07-02). Account settings remain minimal (sign out in header)

Phase-2 exit: signed-in user can save/load cloud projects · OAuth failure does not block local use · RLS tests pass. → **Phase 2 complete 2026-07-02** (live-verified with anonymous auth; Google provider config in the dashboard is the one remaining user step).
