...
import { emptySourceMap, recordBlockLines, type CodeSourceMap } from './sourceMap';
...
function printStatement(stmt: StatementIR, depth: number, state: PrinterState, lines: string[]): void {
  const start = lines.length + 1; // 1-indexed first line this statement emits
  printStatementBody(stmt, depth, state, lines);
  if (stmt.sourceBlockId !== undefined && lines.length >= start) {
    recordBlockLines(state.map, stmt.sourceBlockId, start, lines.length);
  }
}

function printStatementBody(stmt: StatementIR, depth: number, state: PrinterState, lines: string[]): void {
...
export function printArduino(program: ProgramIR): PrintResult {
  ...
  const state: PrinterState = { usedNames: collectUsedNames(program), repeatDepth: 0, map: emptySourceMap() };
  const lines: string[] = [];
  ...
  return { code: lines.join('\n') + '\n', diagnostics: analysis.diagnostics, sourceMap: state.map };
}
